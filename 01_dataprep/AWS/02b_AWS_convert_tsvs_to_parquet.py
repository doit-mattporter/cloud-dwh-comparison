...
for chrom in [x for x in range(1, 23)] + ["X", "Y"]:
    ...
    input_path = f"s3://{output_bucket}/dbNSFP/dbNSFP4.4a_variant.chr{chrom}.tsv"
    output_parquet_path = (
...
spark.conf.set("spark.sql.shuffle.partitions", 200)
input_path = f"s3://{output_bucket}/dbNSFP/dbNSFP4.4_gene_complete.tsv"
output_parquet_path = f"s3://{output_bucket}/dbNSFP/dbNSFP4.4_gene_complete.parquet"
print(f"Converting {input_path} to {output_parquet_path}")
df = spark.read.csv(input_path, sep="\t", header=True)
