...
WITH total_count AS (
  SELECT COUNT(*) AS cnt FROM `PROJECT_ID.DATASET.gnomad_with_dbnsfp_annotations`
)
SELECT info_af_range, COUNT(*) AS count,
       ROUND((COUNT(*) / total_count.cnt) * 100, 3) AS info_af_range_percentage
FROM `PROJECT_ID.DATASET.gnomad_with_dbnsfp_annotations`, total_count
...
FROM `PROJECT_ID.DATASET.gnomad_with_dbnsfp_annotations`
GROUP BY chromosome
LIMIT 24;
...
FROM `PROJECT_ID.DATASET.gnomad_with_dbnsfp_annotations`, total_dbnsfp_count
WHERE clinvar_clnsig IS NOT NULL
...
WITH AF_Differences AS (
    SELECT
        ...
        clinvar_hgvs
    FROM
        `PROJECT_ID.DATASET.gnomad_with_dbnsfp_annotations`
    WHERE
...
FROM
    AF_Differences ad
JOIN
    `PROJECT_ID.DATASET.dbNSFP_gene` g
ON
